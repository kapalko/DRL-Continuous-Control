...
UPDATE_EVERY = 1       # how often the networks update
...
class Agent():
    ...
    def step(self, states, actions, rewards, next_states, dones, step_count):
        ...
        if len(self.memory) > BATCH_SIZE and step_count % UPDATE_EVERY == 0:
    ...
    def learn(self, experiences, gamma):
        ...
        Q_targets_next = self.critic_target(next_states.to(device), actions_next.to(device))  # maps the (state, action) pairs to the Q-values
        ...
        self.noise.reset() # reset noise after learning
